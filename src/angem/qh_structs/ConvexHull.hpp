...
namespace quickhull {
	...
	template<typename T>
	class ConvexHull {
		std::unique_ptr<std::vector<Point<3,T>>> m_optimizedVertexBuffer;
	...
	public:
		...
		ConvexHull(const ConvexHull& o) {
			...
			if (o.m_optimizedVertexBuffer) {
				m_optimizedVertexBuffer.reset(new std::vector<Point<3,T>>(*o.m_optimizedVertexBuffer));
		...
		ConvexHull(const MeshBuilder<T>& mesh, const VertexDataSource<T>& pointCloud, bool CCW, bool useOriginalIndices) {
			if (!useOriginalIndices) {
				m_optimizedVertexBuffer.reset(new std::vector<Point<3,T>>());
			}
		...
		void writeWaveformOBJ(const std::string& filename, const std::string& objectName = "quickhull")
		{
			...
			const size_t triangleCount = indBuf.size() / 3;
