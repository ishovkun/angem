...
#include "Plane.hpp"
#include "qh_structs/Pool.hpp"
#include "qh_structs/Mesh.hpp"
#include "qh_structs/ConvexHull.hpp"
#include "qh_structs/HalfEdgeMesh.hpp"
...
namespace angem {
	...
	public:
		...
		ConvexHull<FloatType> getConvexHull(const std::vector<Point<3,FloatType>>& pointCloud, bool CCW, bool useOriginalIndices, FloatType eps = 1e-4);
		...
		//      then we generate a new vertex buffer which contains only the vertices that are part of the convex hull.
		//   eps: minimum distance to a plane to consider a point being on positive of it (for a point cloud with scale 1)
		ConvexHull<FloatType> getConvexHull(const Point<3,FloatType>* vertexData, size_t vertexCount, bool CCW, bool useOriginalIndices, FloatType eps = defaultEps<FloatType>());
