...
namespace angem {
    ...
    template<typename T>
    ConvexHull<T> QuickHull<T>::getConvexHull(const std::vector<Point<3,T>>& pointCloud, bool CCW, bool useOriginalIndices, T epsilon) {
        VertexDataSource<T> vertexDataSource(pointCloud);
        return getConvexHull(vertexDataSource,CCW,useOriginalIndices,epsilon);
    }

    template<typename T>
    ConvexHull<T> QuickHull<T>::getConvexHull(const Point<3,T>* vertexData, size_t vertexCount, bool CCW, bool useOriginalIndices, T epsilon) {
    ...
    void QuickHull<T>::buildMesh(const VertexDataSource<T>& pointCloud, bool, bool, T epsilon) {
        ...
        // The planar case happens when all the points appear to lie on a two dimensional subspace of R^3.
        m_planar = false;
    ...
    void QuickHull<T>::createConvexHalfEdgeMesh() {
        ...
        while (!faceList.empty()) {
            ...
            if (iter == std::numeric_limits<size_t>::max()) {
                ...
                // At this point we have to reset iteration counter. This shouldn't be an
            ...
            // to solve horizon edge for this point and accept a minor degeneration in the convex hull.
            ...
            for (size_t i = 0; i < horizonEdgeCount; i++) {
                // std::cout << "i = " << i << " of " << horizonEdgeCount << std::endl;
                ...
                // const Point<3,T> planeNormal = mathutils::getTriangleNormal(m_vertexData[A],m_vertexData[B],activePoint);
                // newFace.m_P = Plane<T>(planeNormal,activePoint);

                // dirty hack
                try {
                    newFace.m_P = Plane<T>(m_vertexData[A], m_vertexData[B], activePoint);
                }
                catch (const std::invalid_argument& e)
                {
                    // auto activePoint_d = activePoint;
                    // activePoint_d += 1e-4;
                    // newFace.m_P = Plane<T>(m_vertexData[A], m_vertexData[B], activePoint_d);
                    continue;
                }
                newFace.m_he = AB;
    ...
    std::array<IndexType,6> QuickHull<T>::getExtremeValues() {
        ...
        for (size_t i=1;i<vCount;i++) {
            const Point<3,T>& pos = m_vertexData[i];
    ...
    MeshBuilder<T> QuickHull<T>::getInitialTetrahedron()
    {
        ...
        if (vertexCount <= 4) {
            ...
            // const Point<3,T> N = mathutils::getTriangleNormal(m_vertexData[v[0]],m_vertexData[v[1]],m_vertexData[v[2]]);
            // const Plane<T> trianglePlane(N,m_vertexData[v[0]]);
            const Plane<T> trianglePlane( m_vertexData[v[0]], m_vertexData[v[1]],m_vertexData[v[2]] );
            // if (trianglePlane.isPointOnPositiveSide(m_vertexData[v[3]])) {
            if (trianglePlane.above( m_vertexData[v[3]] ))
                std::swap(v[0],v[1]);
        ...
        for (size_t i=0; i<6; i++) {
            for (size_t j=i+1;j<6;j++) {
                ...
                T d = m_vertexData[ m_extremeValues[i] ].distance( m_vertexData[ m_extremeValues[j] ] );
                d = d*d;
                if (d > maxD) {
                    maxD = d;
        ...
        for (size_t i=0;i<vCount;i++) {
            ...
            T distToRay = r.distance(m_vertexData[i]);
            distToRay *= distToRay;
            if (distToRay > maxD) {
                maxD=distToRay;
                maxI=i;
        ...
        const Point<3,T> baseTriangleVertices[]={ m_vertexData[baseTriangle[0]], m_vertexData[baseTriangle[1]],  m_vertexData[baseTriangle[2]] };
        ...
        maxD = m_epsilon;
        maxI = 0;
        // const Point<3,T> N = mathutils::getTriangleNormal(baseTriangleVertices[0],baseTriangleVertices[1],baseTriangleVertices[2]);
        ...
        if (maxD == m_epsilon) {
            ...
            m_planar = true;
            Plane<T> pln( baseTriangleVertices[1], baseTriangleVertices[2], baseTriangleVertices[0] );
            const Point<3,T> N = pln.normal();
        ...
        const Point<3,T> N = trianglePlane.normal();
        assert( N.norm() != 0 );
        const Plane<T> triPlane(baseTriangleVertices[0], N/N.norm());
        if (triPlane.above(m_vertexData[maxI])) {
        ...
        for (auto& f : mesh.m_faces) {
            ...
            const Point<3,T>& va = m_vertexData[v[0]];
            const Point<3,T>& vb = m_vertexData[v[1]];
            const Point<3,T>& vc = m_vertexData[v[2]];
            const Plane<T> trianglePlane( va, vb, vc );
            // const Point<3,T> N = mathutils::getTriangleNormal(va, vb, vc);
